...
const BookingForm = ({price,propertyName,address,maximumGuest,propertyId,currentBookings}) => {
  ...
  const handleDateChange = (value,dateString) =>{
    ...
    handleFilterChange("chechkoutDate",dateString[1]);
  ...
  currentBookings.forEach((dates)=>
   disabledDate.push({start:dates.fromDate,end:dates.toDate })
  );
  const isDateDisabled = (current) =>{
    if(!disabledDate.length){
      return current.isBefore(Date.now(),"day");
    }else{
      return disabledDate.some((date)=>{
